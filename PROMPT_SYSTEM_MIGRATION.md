...
prompt_manager.py         ← load / save / revert / render
```
...
## Step 3 — Create `prompt_manager.py`
...
In the Streamlit entry point (typically `app.py`), load all prompts once at startup and store them in `st.session_state`. Use the existing startup/caching pattern of the project.
...
1. **Syntax** — all modified files parse without errors:
   ```bash
   python -c "import ast; [ast.parse(open(f).read()) for f in ['prompt_manager.py', 'app.py', 'core.py']]"
   ```
...
- If the other project does not use Streamlit, the `prompt_manager.py` module (Steps 2–3) is framework-agnostic and can be reused as-is. Only the config page UI (Step 6) is Streamlit-specific.
